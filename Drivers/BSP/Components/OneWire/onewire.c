 ...
 */
static void OneWire_Pin_Level(OneWire_t* OW, GPIO_PinState Level) {
#ifdef LL_Driver
	if (Level == 1) {
...
static void OneWire_WriteBit(OneWire_t* OW, uint8_t bit) {
	if (bit) {
		/* Set line low */
		OneWire_Pin_Level(OW, GPIO_PIN_RESET);
...
void OneWire_Init(OneWire_t* OW) {
	OneWire_Pin_Level(OW, GPIO_PIN_SET);
	OneWire_Pin_Mode(OW, Output);
	DwtDelay_us(1000);
